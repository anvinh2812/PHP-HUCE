<?php
	...
	$sql = "SELECT * FROM 0209266_product_31 p INNER JOIN 0209266_categories_31 c on c.cid = p.cid";
...
<body>
		...
		<?php
			$sql2 = "SELECT * FROM 0209266_categories_31 order by cid";
			$result2 = $conn->query($sql2);
		?>
		<div class="menu">
			<ul>
				<li class="all"><a href="t.php">All Categories</a></li>
				<?php
					while($row2 = $result2->fetch_assoc()){
				?>
				<li class='<?php echo $row2["cname"];?>' ><a href='t.php?cid=<?php echo $row2["cid"];?>'><?php echo $row2["cname"];?></a></li>
				<?php
					}
				?>
			</ul>
	...
	<ul class="product-list">
		<?php
			if(isset($_GET["cid"])){
				$cid = $_GET["cid"];
				$sql = "SELECT * FROM 0209266_product_31 p INNER JOIN 0209266_categories_31 c on c.cid = p.cid where p.cid = $cid";
				$result = $conn->query($sql);
			}
